(function () {
  ...
  function hydrateProfileSummary(state) {
    ...
    if (followingNode) {
      ...
      followingNode.textContent = count ? `Siguiendo a ${count} perfil${count === 1 ? "" : "es"}` : "Aún no sigues a nadie";
    }
  ...
  function bindFollow(state) {
    ...
    form.addEventListener("submit", (event) => {
      ...
      if (!candidate) {
        setFeedback(feedbackNode, "No encontramos un perfil compatible con tu búsqueda.", "error");
        return;
      }
  ...
  function findFollowCandidate(state, query) {
    ...
    return allUsers.find((user) => {
      ...
      if (state.user.segment === "adult" && user.segment === "minor") {
        // Adultos pueden seguir a otros adultos y perfiles menores sólo si están aprobados; conservadoramente limitamos a adultos.
